...
class PayoutListCreateView(APIView):
    ...
    def post(self, request):
        ...
        if not idempotency_key:
            return Response(
                {"error": "missing_idempotency_key"}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        if not bank_account:
            return Response(
                {"error": "invalid_bank_account"}, status=status.HTTP_400_BAD_REQUEST
            )

        if amount_paise <= 0:
            return Response(
                {"error": "invalid_amount"}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        with transaction.atomic():
            ...
            transaction.on_commit(
                lambda: process_payout.apply_async(args=[str(payout.id)])
            )
...
class LedgerListView(APIView):
    def get(self, request):
        merchant = current_merchant(request)
        queryset = LedgerEntry.objects.filter(merchant=merchant).order_by(
            "-created_at"
        )[:50]
...
class BankAccountListView(APIView):
    def get(self, request):
        merchant = current_merchant(request)
        queryset = BankAccount.objects.filter(merchant=merchant).order_by(
            "-is_primary", "name"
        )
...
class AddCreditsView(APIView):
    def post(self, request):
        ...
        return Response({"success": True, "amount_paise": amount_paise})
